...
# with open(Poll_answers, "w") as txt_file:
    ...
    for candidate in Candidate_list:
        ...
        #print(f"{candidate}: {Perentage_won:.1f}% ({Candidate_dict[candidate]:,})\n") # dict[candidate] = all candidates w votes, candiate itself allows us to get their values when loop iterates bc in for loop
...
with open(Poll_answers,"w") as file:
    ...
    # file.write (f"Total Votes: {Total_cast} \n"
    #                 f"Candidate Names: {Candidate_list} \n"
    #                 f"Total Votes Won: {Candidate_dict}\n"                  
    #                 f"Percentage Votes Won: {Percentage_list} \n"
    #                 f"Winner: {Winner}")
